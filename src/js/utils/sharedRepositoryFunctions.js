...
export async function addItemToArrayField(
  db,
  uuid,
  itemToAdd,
  collectionToAdd,
...
export async function removeItemFromArrayField(
  db,
  uuid,
  itemToRemove,
  collectionToAdd,
...
export async function setField(db, uuid, collectionToAdd, field, value) {
  ...
  try {
    ...
    console.error(
      `Error setting the field ${field} in the ${collectionToAdd} collection`
    );
    throw error;
  }
}
...
export async function removeDocumentFromCollection(
  ...
  try {
    ...
    console.log(
      `Successfully removed ${singleName} with ID: ${documentId} from collection ${collectionToRemove}`
    );
  } catch (error) {
    console.error(
      `Error removing ${singleName} with ID: ${documentId} from collection ${collectionToRemove}: ${error}`
    );
    throw error;
  }
}
...
export async function getAllItems(db, collectionName, dataConverter = null) {
  const collectionRef =
    dataConverter == null
      ...
      : collection(db, collectionName).withConverter(dataConverter);
...
export async function getArrayFieldFromCollection(
  db,
  collectionName,
  docId,
  fieldName
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.error(
        `Document with ID '${docId}' not found in the collection '${collectionName}'.`
      );
      return [];
    }
  } catch (error) {
    console.error(
      `Error fetching ${fieldName} from collection ${collectionName}:`,
      error
    );
    throw error;
  }
...
export async function updateNonArrayDocumentFields(
  db,
  documentId,
  collectionName,
  fieldsToUpdate
  ...
  try {
    ...
    for (const field in fieldsToUpdate) {
      ...
      if (!fieldExists)
        throw new Error(
          `The field ${field} does not exist in the ${collectionName} collection`
        );
    ...
    console.log(
      `Successfully updated document with ID ${documentId} in collection ${collectionName}`
    );
  } catch (error) {
    console.error(
      `Error updating document with ID ${documentId} in collection ${collectionName}: ${error}`
    );
    throw error;
  }
}

export async function updateArrayDocumentFields(
  db,
  documentId,
  collectionName,
  fieldsToUpdate
