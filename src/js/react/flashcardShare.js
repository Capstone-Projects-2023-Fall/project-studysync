...
function FlashcardShare() {
    ...
    useEffect(() => {
        const fetchFlashcards = async () => {
            try {
                ...
                setCards(Object.keys(flashcardData).map(key => ({
    ...
    const handleLikeClick = async (commentId) => {
        try {
            ...
            const isLiked = commentToUpdate.likedBy.includes(userId);
    
            
            await FlashcardRepo.updateLikesForComment(flashcardId, commentId, userId);
