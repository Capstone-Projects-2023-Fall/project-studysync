...
export class FlashCardRepository {
    ...
    async getFlashcardSet(id) {
        const flashCardSet = await getItemById(
            this.database,
            id,
            "flashcardSets",
            "flashcardSet"
    ...
    async addFlashCardSet(flashcardSet) {
        try {
            ...
            who created it */
            await this.userRepository.addOwnedFlashcard(
                flashcardSet.authorId,
                flashcardRef.id
            );
    ...
    async updateArrayFlashcardSetFields(id, toUpdate) {
        await updateArrayDocumentFields(
            this.database,
            id,
            "flashcardSets",
            toUpdate
    ...
    async getAll_flashcardSets() {
        const collectionRef = collection(this.database, "flashcardSets");
        try {
    ...
    async getFlashcardSetBy_Id(id) {
        const flashcardSets = await this.getAll_flashcardSets();
        for (const flashcardSet of flashcardSets) {
            if (flashcardSet.id == id) return flashcardSet;
        }
        throw new Error(`Flashcard set with id: ${id} does not exist`);
    }
}
